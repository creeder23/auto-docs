# ENVI_SpectralAdaptiveCoherenceEstimator
...
```python
# Quickstart example for ENVI_SpectralAdaptiveCoherenceEstimator.
```
...
input_raster|True|None| |Specify a raster on which to perform the Adaptive Coherence Estimator. -- Value Type: ENVIRASTER
...
covariance|False|None| |Specify an array that is the covariance matrix of the input bands. The array size must be [number of bands,number of bands] -- Value Type: DOUBLE[*, *]
mean|False|None| |Specify an array that is the mean of the input bands. The number of elements in the array must match the number of bands. -- Value Type: DOUBLE[*]
